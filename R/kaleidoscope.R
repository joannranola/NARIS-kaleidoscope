...
draw_shadow = function(t)
{
    ...
    lines(setdiff((x-length(yy)):(x+length(yy)),0), yy[c(length(yy):1, 1:length(yy))], lwd=0.001, col="#FFFFCC")
...
draw_density = function(t, density, generators, generation, colors, scenario='c_RT_R30P', legend_color='white', type='None', density_limits = 'Null', ...)
{
    ...
    if (type == "None") # plot the map without density heatmap
    {
      ...
      raster::plot(density, col="#9F9F9F", border="#606060", lwd=0.001)
      return()
    }
    ...
    d <- kde2dw(x=g$lon, y=g$lat, w=g$power, n=500, lims=as.vector(extent(density)))#n=500
...
{
    edges = unlist(lapply(strsplit(as.character(df$edge), ' '), function(l) { c(l[3], l[1]) }))

    g = igraph::make_empty_graph()
    g = g + igraph::vertices(verts)
    g = g + igraph::edges(edges)
    
    plot(g, layout=layout, edge.curved=0.2, rescale=F, add=T, vertex.size=100, vertex.color='#00000000', vertex.frame.color='#00000000', vertex.label=NA, edge.width=df$weight, edge.color=edge_color, edge.arrow.size=df$weight/arrow.scaling)
}
